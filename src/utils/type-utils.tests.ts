...
assert<IsExact<DistributivePick<Entity, DistributiveKeyof<Entity>>, Entity>>(
  true
);
...
assert<Has<DistributivePick<Entity, "a" | "b">, Pick<EntityA, "a" | "b">>>(
  true
);
...
assert<
  IsExact<
    PickPath<Record<string, string>, "any key">,
...
assert<IsExact<NonNullable<PickPath<EntityC, "k.m">["k"]>, { m: number }>>(
  true
);
